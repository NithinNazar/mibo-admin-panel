...
const CentreManagersPage: React.FC = () => {
  ...
  const fetchManagers = async () => {
    try {
      ...
      const data = await staffService.getStaffByRole(3);
      setManagers(data);
    } catch (error: any) {
      toast.error("Failed to fetch centre managers");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateManager = async () => {
    try {
      ...
      toast.success("Centre Manager created successfully!");
  ...
  const handleViewDetails = async (manager: CentreManager) => {
    try {
  ...
  const handleToggleActive = async (id: string, isActive: boolean) => {
    try {
      ...
      toast.success(
        `Centre Manager ${isActive ? "activated" : "deactivated"} successfully`,
      );
      fetchManagers();
    } catch (error: any) {
      toast.error("Failed to update centre manager status");
    }
  };
  ...
  const handleExportCSV = () => {
    const csvData = managers.map((manager) => ({
      ...
      Designation: manager.designation || "Centre Manager",
      Status: manager.isActive ? "Active" : "Inactive",
    }));
    exportToCSV(csvData, "centre-managers");
  ...
  const handleExportPDF = () => {
    ...
    const rows = managers.map((manager) => [
      ...
      manager.designation || "Centre Manager",
      manager.isActive ? "Active" : "Inactive",
    ]);
    exportToPDF(headers, rows, "Centre Managers List");
  ...
  const handlePrint = () => {
    ...
    printTable("Centre Managers List", headers, rows);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-white">Centre Managers</h1>
          <p className="text-slate-400 mt-1">Manage centre-level managers</p>
        </div>
        <Button
          variant="primary"
      ...
      {/* Create Centre Manager Modal */}
      <Modal
        isOpen={showCreateModal}
        onClose={() => setShowCreateModal(false)}
        title="Add New Centre Manager"
      >
        <div className="space-y-4">
          ...
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
            <p className="text-sm text-blue-400">
              ...
              securely. The centre manager will use these credentials to log in.
            </p>
          </div>
...
export default CentreManagersPage;
